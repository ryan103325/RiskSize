...
計算已含 **手續費（可設折數）** 與 **現股當沖證交稅 0.15%**；止盈價位以
**扣除費用後的淨損益** 反推，確保實際淨盈虧比精確等於設定值（例如 1:2 就是真的 1:2）。
...
- ✅ 淨盈虧比：止盈價含交易費用反推，預期浮盈 = 盈虧比 × 實際動用風險金額，不會因費用而低於設定的盈虧比
...
單股總風險 = 止損距離 + 單股成本      ← 止損時的實際淨虧損
單張總風險 = 單股總風險 × 1000
可持有張數 = floor(風險金額 × 1000 ÷ 單張總風險)

止盈價位（含費用反推，使 淨獲利 = 盈虧比 × 淨虧損）：
  做多 = (進場價 × (1 + 手續費率) + 盈虧比 × 單股總風險) ÷ (1 − 手續費率 − 稅率)
  做空 = (進場價 × (1 − 手續費率 − 稅率) − 盈虧比 × 單股總風險) ÷ (1 + 手續費率)

預期浮盈 = 盈虧比 × 實際動用風險金額（精確成立）
```

> 註：原規格書 3.3 的止盈價為「進場價 ± 盈虧比 × 止損距離」，扣除手續費與證交稅後
> 淨盈虧比會低於設定值；本實作改以淨損益反推止盈價，讓盈虧比是「稅後費後」的真實比例。
